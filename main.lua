...
local window = library:AddWindow("Script Private Jeff CRACK FOR SN | Bienvenido " .. displayName, {
    main_color = Color3.fromRGB(139, 0, 0),
    min_size = Vector2.new(630, 900),
    can_resize = false,
})
...
AutoFarm:AddSwitch("Anti AFK", function(state)
    if state then
        ...
        uselesslabelone.Text = "Anti Afk CRACK for SN"
...
autoEquipToolsFolder:AddSwitch("Auto Pushups y Pegarle Ala Roca de 10M", function(state)
...
rebirthsFolder:AddTextBox("Rebirth Target", function(text)
    ...
    if newValue and newValue > 0 then
        ...
        game:GetService("StarterGui"):SetCore("SendNotification", {
            Title = "Entrada InvÃÂ¡lida",
            Text = "Por favor ingresa un nÃÂºmero vÃÂ¡lido mayor que 0",
            Duration = 0
        })
    end
end)
...
end, "Renacimiento automÃÂ¡tico hasta alcanzar el objetivo")
...
end, "Establece el tamaÃÂ±o del personaje a 1 continuamente")
...
end, "Teletransporte continuo al Rey MÃÂºsculo")
...
local function gettool()
    pcall(function()
        ...
        -- Equipar y activar Punch si existe
        if punchTool then
            character.Humanoid:EquipTool(punchTool)
            punchTool:Activate() -- <- Esto hace que se vea la animaciÃ³n
        end

        -- Disparar el evento al servidor para el daÃ±o
        player.muscleEvent:FireServer("punch", "leftHand")
        player.muscleEvent:FireServer("punch", "rightHand")
    end)
end
...
local whitelistedPlayersLabel = killerTab:AddLabel("Whitelist Jugadores: None")
...
local function updateTargetPlayerLabel()
    if _G.targetPlayer == "" then
        targetPlayerLabel.Text = "Blacklist: Ninguno"
    else
...
local autoWhitelistFriendsSwitch = killerTab:AddSwitch("Agregar Amigos a WhiteList", function(bool)
...
killerTab:AddTextBox("Agregar Jugador a Whitelist (User/Nick)", function(text)
...
killerTab:AddTextBox("Quitar Jugador de Whitelist (Name/Nickname)", function(text)
    if text and text ~= "" then
...
killerTab:AddButton("Limpiar Whitelist", function()
...
local spyTargetDropdown = killerTab:AddDropdown("Seleccionar Jugador Para Ver", function(selectedText)
	for _, player in ipairs(Players:GetPlayers()) do
		local combined = string.format("%s (%s)", player.DisplayName, player.Name)
		if combined == selectedText then
...
-- ð¹ Agregar jugadores actuales
for _, player in ipairs(Players:GetPlayers()) do
	if player ~= LocalPlayer then
		local combined = string.format("%s (%s)", player.DisplayName, player.Name)
		spyTargetDropdown:Add(combined)
		spyTargetDropdownItems[player.Name] = combined
	end
end

-- ð¹ Cuando entra un jugador nuevo
...
-- ð¹ Cuando se va un jugador
Players.PlayerRemoving:Connect(function(player)
	if player ~= LocalPlayer then
		spyTargetDropdownItems[player.Name] = nil
		spyTargetDropdown:Clear()
		for _, combined in pairs(spyTargetDropdownItems) do
			spyTargetDropdown:Add(combined)
		end
	end
end)
...
killerTab:AddSwitch("Ver Jugador", function(bool)
    spying = bool
    if not spying then
...
-- â« SISTEMA DE BLACKLIST (Auto Kill)
_G.blacklistPlayers = _G.blacklistPlayers or {}

local function updateBlacklistLabel()
    if #_G.blacklistPlayers == 0 then
        targetPlayerLabel.Text = "Blacklist: Ninguno"
    else
        targetPlayerLabel.Text = "Blacklist: " .. table.concat(_G.blacklistPlayers, ", ")
    end
end

-- ð§¾ Crear dropdown dinÃ¡mico (DisplayName | Username)
local blacklistDropdown = killerTab:AddDropdown("Agregar a BlackList", function(selectedItem)
    local selectedDisplay, selectedUser = selectedItem:match("(.+)%s|%s(.+)")
    if not selectedUser then return end

    for _, plr in ipairs(Players:GetPlayers()) do
        if plr.Name == selectedUser then
            local playerInfo = plr.Name .. " (" .. plr.DisplayName .. ")"
            if not table.find(_G.blacklistPlayers, playerInfo) then
                table.insert(_G.blacklistPlayers, playerInfo)
                game:GetService("StarterGui"):SetCore("SendNotification", {
                    Title = "â« Blacklist",
                    Text = selectedDisplay .. " aÃ±adido a la lista negra",
                    Duration = 3
                })
...
-- ð§ Llenar dropdown con los jugadores actuales
for _, player in ipairs(Players:GetPlayers()) do
    if player ~= LocalPlayer then
...
-- ð Actualizar dropdown cuando entran o salen jugadores
Players.PlayerAdded:Connect(function(player)
    if player ~= LocalPlayer then
        blacklistDropdown:Add(player.DisplayName .. " | " .. player.Name)
    end
...
Players.PlayerRemoving:Connect(function(player)
    ...
    for _, plr in ipairs(Players:GetPlayers()) do
        if plr ~= LocalPlayer then
            blacklistDropdown:Add(plr.DisplayName .. " | " .. plr.Name)
        end
    end
end)

-- ðï¸ Quitar el Ãºltimo jugador agregado
killerTab:AddButton("Quitar Ãltimo de BlackList", function()
    if #_G.blacklistPlayers > 0 then
        local removed = table.remove(_G.blacklistPlayers)
        game:GetService("StarterGui"):SetCore("SendNotification", {
            Title = "âª Blacklist",
            Text = "Eliminado: " .. removed,
            Duration = 3
        })
        updateBlacklistLabel()
    else
        game:GetService("StarterGui"):SetCore("SendNotification", {
            Title = "âª Blacklist",
            Text = "No hay jugadores en la lista.",
            Duration = 3
        })
    end
end)

-- ð§¹ Limpiar la BlackList completa
killerTab:AddButton("Limpiar BlackList", function()
    _G.blacklistPlayers = {}
    updateBlacklistLabel()
    game:GetService("StarterGui"):SetCore("SendNotification", {
        Title = "â Blacklist",
...
-- âï¸ AUTO KILL DE BLACKLIST
killerTab:AddSwitch("Matar BlackList", function(state)
    _G.autoKillBlacklist = state

    if state then
        task.spawn(function()
            while _G.autoKillBlacklist do
                pcall(function()
                    for _, info in ipairs(_G.blacklistPlayers) do
...
killerTab:AddSwitch("Auto Kill A Todos", function(bool)
    autoKill = bool
...
local button = killerTab:AddButton("Quitar la animaciÃ³n al puÃ±o", function()
...
killerTab:AddButton("Regresar La Animacion Al PuÃ±o", function()
    RecoveryPunch()
end)
...
-- ð Teleport / Follow System (versiÃ³n auto-follow desde Dropdown)
...
-- ð FunciÃ³n: TP detrÃ¡s del jugador
...
-- ð Dropdown dinÃ¡mico de jugadores
local followDropdown = killerTab:AddDropdown("Teleport player", function(selectedDisplayName)
    if selectedDisplayName and selectedDisplayName ~= "" then
        ...
        if target then
            ...
            following = true
            print("â Started following:", target.Name)
...
-- ð Inicializar lista con DisplayNames
for _, player in ipairs(Players:GetPlayers()) do
    if player ~= LocalPlayer then
...
-- ð Actualizar lista cuando entren jugadores
Players.PlayerAdded:Connect(function(player)
    if player ~= LocalPlayer then
        followDropdown:Add(player.DisplayName)
    end
...
-- ð Actualizar lista cuando se vayan jugadores
Players.PlayerRemoving:Connect(function(player)
...
-- ð BotÃ³n para dejar de seguir
killerTab:AddButton("Dejar de Seguir", function()
    following = false
    followTarget = nil
    print("â Stopped following")
end)

-- ð Loop de seguimiento automÃ¡tico
task.spawn(function()
...
-- ð Reintentar cuando respawnees
...
killerTab:AddSwitch("DaÃ±o con God mode", function(state)
...
-- Ã°ÂÂÂ Lista de RAWs a ejecutar
local urls = {
...
-- Ã¢ÂÂ¡ BotÃÂ³n que ejecuta todos los scripts remotos
killerTab:AddSwitch("Pegar Muerto", function()
...
local button = killerTab:AddButton("Congelar el agua", function()
        ...
        if SelectPlayerName ~= "" then
            ...
            if player then
                ...
                if player:FindFirstChild("currentMap") then
                    ...
                    Update9.Text = "Current Map: Aucune donnÃÂÃÂÃÂÃ¢ÂÂÃÂÃ¢Â ÃÂ¢Ã¢ÂÂ¬Ã¢ÂÂ¢ÃÂÃÂÃÂ¢Ã¢ÂÂ¬ÃÂ¡ÃÂÃ¢ÂÂÃÂÃÂ©e"
                end
            end
        end
    end
...
local playerDamageLabel = estadisticas:AddLabel("Tu DaÃ±o: N/A")
...
-- ð§® FunciÃ³n para calcular la vida total del jugador enemigo
...
-- ð§  CÃ¡lculo de golpes necesarios
local function calculateHitsToKill(health, damage)
    if damage <= 0 then return "â" end
...
local function updateAdvancedStats(targetPlayer)
    ...
    playerDamageLabel.Text = "Tu DaÃ±o: " .. FormatDisplay(playerDamage)
    hitsToKillLabel.Text = "Golpes para Matar: " .. tostring(hitsToKill)
end

-- ð Bucle de actualizaciÃ³n vinculado al jugador seleccionado
task.spawn(function()
...
Credits:AddLabel("script JEFF cracked by SN")
Credits:AddLabel("SCRIPT ECHO POR")
Credits:AddLabel("Rafael robado por JEFF")
